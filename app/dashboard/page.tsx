          ...
          <div className="flex flex-col items-center justify-center mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full mb-8">
              ...
              <div className="bg-gray-800 p-6 rounded-lg shadow-lg transform hover:scale-105 transition-transform duration-300">
                <h2 className="text-xl font-semibold mb-3 text-blue-400">Total Requests</h2>
                {loadingStats ? <div className="h-10 bg-gray-700 rounded animate-pulse"></div> : <p className="text-5xl font-bold">{requestStats?.totalRequests}</p>}
              </div>

              {/* Requests by Status */}
              <div className="bg-gray-800 p-6 rounded-lg shadow-lg md:col-span-2 transform hover:scale-105 transition-transform duration-300">
                <h2 className="text-xl font-semibold mb-4 text-green-400">Requests by Status</h2>
                {loadingStats ? (
                    <>
            ...
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full mb-8">
              <h2 className="text-xl font-semibold mb-4 text-purple-400">Active Migrations</h2>
              {loadingMigrations ? (
                  ...
                  <table className="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                      {migrationList.length > 0 ? migrationList.map(mig => (
                        <tr key={mig.id} className="hover:bg-gray-750 transition-colors duration-150">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100">{mig.id}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{mig.name}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              mig.status === 'Approved' ? 'bg-green-600 text-green-100' :
                              mig.status === 'Pending' ? 'bg-yellow-600 text-yellow-100' :
                              mig.status === 'Rejected' ? 'bg-red-600 text-red-100' :
                              mig.status === 'In Progress' ? 'bg-blue-600 text-blue-100' :
                              'bg-gray-600 text-gray-100' // Completed or other
                            }`}>
                              {mig.status}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{mig.submittedBy}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{mig.submittedDate}</td>
                        </tr>
                      )) : (
                        <tr>
                            <td colSpan={5} className="text-center py-4 text-gray-500">No migrations to display.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                    ...
                    <ul className="space-y-3">
                        <li className="text-sm text-gray-400 border-b border-gray-700 pb-2">User 'admin' approved Change Request #CR123. <span className="text-xs text-gray-500 float-right">2 hours ago</span></li>
                        <li className="text-sm text-gray-400 border-b border-gray-700 pb-2">New Change Request #CR126 submitted by 'dev_user'. <span className="text-xs text-gray-500 float-right">1 day ago</span></li>
                        <li className="text-sm text-gray-400">Migration 'MIG003' status updated to 'In Progress'. <span className="text-xs text-gray-500 float-right">3 days ago</span></li>
                    </ul>
                </div>
                <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                    <h2 className="text-xl font-semibold mb-4 text-indigo-400">Quick Links</h2>
                    <ul className="space-y-2">
                        <li><a href="/change-management/new" className="text-blue-400 hover:underline hover:text-blue-300 transition-colors">Submit New Change Request</a></li>
                        <li><a href="/admin-config/users" className="text-blue-400 hover:underline hover:text-blue-300 transition-colors">Manage Users</a></li>
                        <li><a href="/reports" className="text-blue-400 hover:underline hover:text-blue-300 transition-colors">View System Reports</a></li>
                        <li><a href="/profile" className="text-blue-400 hover:underline hover:text-blue-300 transition-colors">My Profile</a></li>
                    </ul>
                </div>
            </div>
