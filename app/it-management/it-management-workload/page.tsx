...
interface TaskCardProps {
  task: Task;
  provided: any; // DnD provided props
  snapshot: any; // DnD snapshot
}
...
export default function ManagementTasks() {
  ...
  const handleExportToExcel = useCallback(async (): Promise<void> => {
    try {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute('download', `it-management-tasks-${timestamp}.csv`);
  ...
  const handlePost = useCallback(async (task: Omit<Task, 'id'>): Promise<void> => {
    ...
    try {
      ...
      console.log("📤 IT Management - Final payload being sent:", JSON.stringify(payload, null, 2));
      ...
      console.log("✅ IT Management - Raw response:", responseText);
      
      let newTask: ApiTask;
      try {
        newTask = JSON.parse(responseText);
        console.log("✅ IT Management - Parsed task:", newTask);
      } catch {
  ...
  const handleSave = useCallback(async (task: Task): Promise<void> => {
    ...
    try {
      ...
      console.log('📤 IT Management UPDATE - Payload being sent:', payload);
  ...
  const TaskCard = memo(({ task, provided, snapshot }: TaskCardProps) => (
    <div
        ...
        <div className="flex-1 md:ml-60 p-6">
          ...
          {loading ? (
            <div className="flex justify-center">
              <p className="text-gray-500 dark:text-gray-400">Loading management tasks...</p>
            </div>
          ) : (
